	...
	// ===========================================================================
			...
			always @(negedge CLK) begin
					if(RST == 1'b1) begin
							...
							case(pState)
									Idle : begin wSR <= DIN; end               // load write register prior to Init
											
									Init : begin wSR <= wSR; end
									
									RxTx : begin 
											if(CE == 1'b1) wSR <= {wSR[6:0], 1'b0}; //shift out the write bit that just got read by the slave
									        else wSR <= wSR;
									end
											
									Done : begin wSR <= wSR; end				
			...
			always @(posedge CLK) begin
					if(RST == 1'b1) begin
							...
							case(pState)
									...
									RxTx : begin
											if(CE == 1'b1) begin
													rSR <= {rSR[6:0], MISO}; // shift in next bit from slave
											end
									end
					...
					if(RST == 1'b1) begin
							...
							case (pState)
								...
								Idle : begin
										...
										if(sndRec == 1'b1) begin // If send receive signal received begin data transmission
											pState <= Init;
										end
										else begin                // Else just chill
											pState <= Idle;
										end
								...
								Init : begin
										...
										BUSY <= 1'b1;			// Data transmission initialize; bring busy high
										bitCount <= 4'h0;		// Have not read/written anything yet, reset bitCount
										CE <= 1'b0;				// Disable serial clock, don't send the pulse yet
								...
								RxTx : begin
										...
										BUSY <= 1'b1;						// Keep busy signal high
